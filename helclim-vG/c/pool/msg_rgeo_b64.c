...
int geomsg(int NAV_CRES,int NAV_LRES,int NAV_COFF,int NAV_LOFF,int lin,int col,double *lat_ptr,double *lon_ptr,double *satazi_ptr, double *satzen_ptr)
{
...
/* RM May 2008: added equations for the calculation of satellite zenith and azimuth 
                 !! it is assumed that the satellite is at lat=lon=0 */
*satazi_ptr = atan(sin(*lon_ptr)/tan(*lat_ptr));
/* this was the viewing angle, not totally correct anyway */
/* *satzen_ptr = atan(6378.169*sin(*lat_ptr)/36000); */
*satzen_ptr = atan(sin(*lat_ptr) * r_sat/(cos(*lat_ptr) * r_sat - r_e));
...
/* RM May 2008: changed the data type from unsigned long to unsigned int to get the program running 
   on the new Linux 64 bit machine */
    (*temp).nrlines=*((unsigned int *)(&headline[3])); 
    (*temp).nrcolumns=*((unsigned int *)(&headline[7]));
 ...
 /* RM May 2008: changed the data type from signed long to signed int to get the program running 
   on the new Linux 64 bit machine */
    (*temp).line_off=*((signed int *)(&headline[17]));
    (*temp).col_off=*((signed int *)(&headline[21]));
    (*temp).nav_lres=*((signed int *)(&headline[25]));  /* Line  Resolution  */
    (*temp).nav_cres=*((signed int *)(&headline[29]));  /* Column Resolution  */

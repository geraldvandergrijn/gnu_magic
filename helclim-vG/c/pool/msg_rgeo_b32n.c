...
double w;                                   /* hour angle*/
w=lon-lon_sun;
double(solar_azimuth), tmpsazi;
tmpsazi=acos((cos(lat)*sin(dec)-cos(w)*cos(dec)*sin(lat))/sin(sza));
if(sin(w)>0)
{solar_azimuth=2*PI-tmpsazi;} 
else
{solar_azimuth=tmpsazi;}
/* cos_solar_azimuth = (sin(sza)*sin(lat)-sin(dec))/(sin(sza)*cos(lat)); */
         /* approximation see wikipedia  cos(elevation)=sin(pi/2-elevation)= sin(sza)*/

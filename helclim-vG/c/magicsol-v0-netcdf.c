...
#include <netcdf.h>            /* header file for netcdf interface */
...
// begin define the netcdf interface stuff --->
#define NLAT 205  // it accordingly 
#define NLON 405
#define LAT_NAME "lat"
#define LON_NAME "lon"
#define VAR_NAME "data"
#define TIME_NAME "imageTime"
#define DATE_NAME "imageDate"
// end define netcdf stuff
...
/* start with the netcdf interface */ 
   /* Open the file. */
//   if ((retval = nc_open(FILE_NAME, NC_NOWRITE, &ncid)))
     if ((retval = nc_open(in_imagefile, NC_NOWRITE, &ncid)))
      ERR(retval);
   ...
   if ((retval = nc_inq_varid(ncid, TIME_NAME, &time_varid)))
       ERR(retval);
   if ((retval = nc_inq_varid(ncid, SZA_NAME, &sza_varid)))
       ERR(retval); 
/* Get the varids of the variable CI from netCDF file */
   ...
   if ((retval = nc_get_var_float(ncid, lon_varid, &lons_in[0])))
      ERR(retval);
   if ((retval = nc_get_var_float(ncid, time_varid, &time_in[0])))
      ERR(retval); 
   if ((retval = nc_get_var_short(ncid, sza_varid, &sza_in[0][0][0])))
      ERR(retval);

   /* Read the data. Since we know the contents of the file we know
    * that the data arrays in this program are the correct size to
    * hold all the data. */
   if ((retval = nc_get_var_short(ncid, var_varid, &var_in[0][0][0])))
      ERR(retval);

// for testing the netcdf interface:
   // for (ilat = 0; ilat < NLAT; ilat++){
   //  for (ilon = 0; ilon < NLON; ilon++){
//	 printf("the netcdf input %f %f %f %d %d %d %d \n", lats_in[ilat], lons_in[ilon], time_in[40],sza_in[40][ilat][ilon], var_in[1][ilat][ilon], ilat, ilon);
   //   }
   //  }	
   ...
   printf("*** SUCCESS reading netcdf file %s \n", in_imagefile);
   // return 0;
    ...
    /* GMT : Abscannzeitpunkt dieser Zeile bestimmen */
     ...
     /* printf("now here vv \n");*/
	  ...
	  if(MFG==1){
          ...
          lon=lon+dpos/r2d; /* RM Feb11, geoloc (geomsg) assume that the satellite is at 0 deg 

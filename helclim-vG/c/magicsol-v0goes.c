...
/* V1x22 based on University of Oldenburg Heliosat cloudindex scheme 
   ...
   The version is based on heliosat-byte-v01.c, which has been the basis for
    the processing of the first Heliosat CDR by MeteoSwiss. In difference to the applied version, 
    this version comes with the standard method for the calculation of the clear sky reflection.
   MODIFICATIONS:
  ...
  RM Nov 2009         added new formula/equation for the calculate of satzen, the old 
                      ...
                      Changed place where earth-sun distance correction occur for ADM study
   
 
  RM Dez 2009 (MS/DWD version), implemented here  Feb 2011
            The earth-sun distance correction is now done earlier in the program & more consistent with 
            the processing logic.  In parellel a new geometry correction (Albedo correction) for
            Rmax calculation has been implemented. This new correction does not lead to a more 
            stable rmax, yet it is more fancy and probably closer to physics, please see the 
            respective comments within the code.  
            !! The old Rmax correction requires that the earth-sun distance correction is
            done after the Rmax correction, else significant errors occur !!!
  RM Feb 11  Changed the handling of dpos, Eumetsat geolocation routine assumes that the satellite is
             located at 0 longitude. In order to correct for other positions dpos has to be 
             added to the longitude given back by the geoloc routine !!! 
             Satzen and satazi has to be modifierd accordingly ??!!
...
double slope, y_intercept;     /* affine transformation of cloud index to 
                                  short int values */
...
void read_grid_header(short int head_length, char *image_name, unsigned char *headline);

short int **read_xpif_to_matrix(short int xpif_head_length,char *image_name,struct bildinfo *imageinfo);

short int **read_grid_to_matrix(short int head_length, char *image_name, struct headinfo *hinfo);

unsigned char **read_grid_to_Bmatrix(short int head_length,char *image_name,struct headinfo *hinfo);
...
/* calculates ground reflectivity and cloud index 
   A. Hammer 2005 */
short int cloudindex(short int reflectivity,short int ground,
                     short int rhomax,short int shadow);
...
{ 
  ...
  struct headinfo hinfo;    // needed to read the header of the regular lat,lon grids
  struct bildattribut{
  ...
  short int *fper = NULL;  
  int kend1=0;
  /* RM May 2008: added averaged angles used for the caclculation of the 
     scatter angle scata */
  float msza=0;  /* average of the solar zenith angle over time and space 
                    within the calibration region */
  float mazi=0;  /* average of the solar azimuth angle over time and space 
                    within the calibration region */
  ...
  float dpos=0.0; /* dpos is the position of Meteosat relativ to lon=0, 
  ...
  short int **out;           /* Matrix for the output of clear sky reflections    */
  ...
  char *channel="MFGVIS"; /* added, currently hard wired, 
                             should be provided via heliosat call 
                             once the thermal irradiance is included */
  short int MFG=1; /* added, is overwriten by Heliosat call parameter -z */
  int i,k,laenge;
  char quit='\0';
  ...
  *out_imagefile = '\0';
  ...
  for (i=1; i<argc; i++){  /* fuer alle Argumente..                          */
    ...
    *cp=(char) tolower((int) *cp); /* es wird nicht zwischen Klein- und      *
    ...
    else if (*cp=='s') dpos=atof(argv[++i]); /*dpos instead of sig_ground, 
  ...
  if(MFG==1){
    printf("assume MFG image data as input !! \n please check if you realy use MFG: -z has to be 0 for MSG \n");
      ...
      printf("note! binary data on regular lat, lon grid is expected \n header 256 bytes defined in struct headinfo \n header defines lat,lon & so on \n");
  }
  else{
    printf("assume MSG image data as input !! \n please check if you realy use MSG: -z has to be 1 for MFG \n -z has to be 2 for a regular grid \n for any other value MSG XPIF input is assumed \n");
  }
 ...
 /* oeffnen aller Dateien, die in Liste stehen, 
  ...
  while(!feof(list) && anzahl < BILDANZ ){
    ...
    /* store header for output XPIF-images */     
    if (MFG != 99){ 
	read_xpif_header(HDL,bild,&bildfolge[anzahl].headline[0]); 
    }
    ...
    if(MFG==99)
   {
       ...
       // bildfolge[anzahl].imagedata=(short int)read_grid_to_Bmatrix(HDL,bild,&hinfo);
      bildfolge[anzahl].Bimagedata=read_grid_to_Bmatrix(HDL,bild,&hinfo);
       printf("grid header %d %d %f %f %f %f %f %f %d %d %d \n", hinfo.nrlines, hinfo.nrcolumns, hinfo.latbegin, 
hinfo.lonbegin, hinfo.dlon, hinfo.dlat, hinfo.lonend, hinfo.latend, hinfo.aq_time, hinfo.date, hinfo.scandur);


      bildfolge[anzahl].imagedata=smatrix(0,hinfo.nrlines-1,0,hinfo.nrcolumns-1);
       for (lin = 0;lin < hinfo.nrlines;lin++) {  /* change type and rotate the image, afterwards free bmatrix */
        for(col = 0;col < hinfo.nrcolumns;col++){ 
	      /* temp=(short int)bildfolge[anzahl].Bimagedata[geoinfo.nrlines-lin-1][geoinfo.nrcolumns-col-1]; */
	      /* correct if old openMTP converter is used */
	    // h.nrlines-i-1
	  bildfolge[anzahl].imagedata[lin][col]=4*(short int)bildfolge[anzahl].Bimagedata[hinfo.nrlines-lin-1][col]; 
	  /* correct if new openMTP to XPIF converter is used */
          /* bildfolge[anzahl].imagedata[lin][col]=4*(short int)bildfolge[anzahl].Bimagedata[lin][col]; */
	  ...
	  /* multiplication with factor 4 needed in order to have only one BYTE MAX definition */  
       ...
       //      printf("the data %d %d %d  \n",bildfolge[anzahl].imagedata[2000][2000], bildfolge[anzahl].imagedata[1000][1000],bildfolge[anzahl].imagedata[1500][1500] );
      // bildfolge[anzahl].imagedata=read_grid_to_Bmatrix(HDL,bild,&hinfo);
      db_hours_per_line=((float)hinfo.scandur/hinfo.nrlines/60.); 
      free_bmatrix(bildfolge[anzahl].Bimagedata,0,hinfo.nrlines-1,0,hinfo.nrcolumns-1); 
   }
    else
    {
    ...
    if(MFG!=99){
    bildfolge[anzahl].doy=hinfo.date;
    ...
    {
	bildfolge[anzahl].doy=hinfo.date;
	hour=hinfo.aq_time/100;
        min=fmod(hinfo.aq_time,100);
	// bildfolge[anzahl].doy=160;
        //hour=1;
        // min=25;
        geoinfo.nrlines=hinfo.nrlines;
        geoinfo.nrcolumns=hinfo.nrcolumns;
        printf("the doy, hour, min %d %d %d \n", bildfolge[anzahl].doy,hour,min);	
        geoinfo.nrbytes=2;
    }
   ...
   printf("Iam here 1 %d %d \n",geoinfo.nrlines, geoinfo.nrcolumns); 
    ...
    bildfolge[anzahl].EOT=equation_of_time(bildfolge[anzahl].doy,
					   &bildfolge[anzahl].gamma,
					   &bildfolge[anzahl].f,
					   &bildfolge[anzahl].dec);
    zahl++; 
  } 
  n=anzahl;    
...
program crashes */
  ...
  for (lin = 0;lin < geoinfo.nrlines;lin++){   /* fuer alle Zeilen */ 
    /* 12.10.09 included new lines for the calculation of GMT. 
       Now MFG and MSg can be processed */
    ...
    /* GMT : Abscannzeitpunkt dieser Zeile bestimmen */
      ...
      // printf("now here GMT= %f %f \n", GMT, db_hours_per_line );
	  ...
	  if(MFG==1){
	  ...
	  }else if(MFG==99)
          {
              ...
              decide=0;
	     lon=(hinfo.lonbegin+col*hinfo.dlon)*deg2rad;  // /100 needed as dlon is given as int in header
	     // lat=(hinfo.latbegin+lin*hinfo.dlat)*deg2rad;              
             lat=(hinfo.latend-lin*hinfo.dlat)*deg2rad;
	     //printf("lat lon %d %d %f %f %d %d \n",lin,col, lat*r2d, lon*r2d, hinfo.lonbegin, hinfo.dlon);         
    // if lat lon >70 decide=1
          }
          else{
	      decide=geomsg(geoinfo.nav_cres,geoinfo.nav_lres,
			    geoinfo.col_off,geoinfo.line_off,
			    lin,col,&lat,&lon);               
	  }
	  if(MFG!=99){lon=lon+dpos/r2d;} 
	  //          printf("lonp lat lon %d %d %f %f %d %d \n",lin,col, lat, lon, hinfo.lonbegin, hinfo.dlon); 
         /* RM Feb11, geoloc (geomsg) assume that the satellite is at 0 deg 
                            longitude, hence dpos has to be added to the lon given back by the routine */
	  /* if (decide == 0){
	  ...
	  if (decide==0){
		  ...
		  if(coszen>0.02)
		      corr = (int)((bildfolge[k].imagedata[lin][col]-doffset)/(coszen*bildfolge[k].f)); 
		  //if(coszen<0.02){printf("coszen: lat lon %f %f \n", lat*r2d, lon*r2d);}
		  /* orig 51 instead of doffset, RM 0211 shifted earth sun-distance correction
		     .f to this place */
		  ...
		  if(lat > -0.92 && lat < -0.8 && lon  < (-0.01 + dpos/r2d) && lon > (-0.262+dpos/r2d)){
  ...
  /* pre-requisite of the new correction is that the earth-sun distance has been done
     beforehand */
 ...
 /* alternative define own nint function
...
*/
  ...
  printf("the lrint = %d \n", (int)round(dpos/15.0)); 
  if ((hour + (int)round(dpos/15.0))==13 && min==0){ 
  /* RM,feb11 added lrint rounds to nearest integer, needed to get local noon for MetEast */
  ...
  for (lin = 0;lin < geoinfo.nrlines;lin++)   /* fuer alle Pixel (Zeilen ... */
  {
	  ...
	  for(k = 0; k<=n; k++)
	  {
		 ...
		 reflectivity_vector contains all reflectivity values for one location */
	      ...
	      /* here is the old plave of the earth-sun correction ! */
	  } 
 ...
 ***********************************************************************/ 
  ...
  printf("geoinfo nrbytes= %d",geoinfo.nrbytes) ;
...
FILE *in_image;                         /* FILEname des Eingabebildes */
 ...
 if(head_length==256)
 {  
  fread(&headline[0],head_length,1,in_image);
  rheadinfo(&headline[0],hinfo); 
 }
  else
  {
  fprintf(stderr,"Crash: The given headerlength is not 256 or 260!\n");
 ...
 fseek(in_image, (long) (head_length), SEEK_SET);
	imagedata=smatrix(0,(*hinfo).nrlines-1,0,(*hinfo).nrcolumns-1);
	fread(&imagedata[0][0],(*hinfo).nrcolumns*(*hinfo).nrlines*sizeof(char),1,in_image);  //
  fclose (in_image);
  return imagedata;
...
FILE *in_image;                         /* FILEname des Eingabebildes */
 ...
 fseek(in_image, (long) (head_length), SEEK_SET);
	imagedata=bmatrix(0,(*hinfo).nrlines-1,0,(*hinfo).nrcolumns-1);
	fread(&imagedata[0][0],(*hinfo).nrcolumns*(*hinfo).nrlines*sizeof(char),1,in_image);  //
  fclose (in_image);
  return imagedata;

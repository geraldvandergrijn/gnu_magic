...
int main(void)
{
 ...
 Version: 0.9
 Nov 12: added functions to read and treat MAGIC standard binary header, 
 needed for the geolocation and processing of the data from GOES satellites */
...
**/
  /* the program starts with the declaration of the needed variables 
   and parameters */
  struct headinfo hinfo; /* added RM nov12  */
  ...
  struct image{
    ...
    unsigned char header[HDL];    /* header, changed length of header & char-> unsigned char, RM Nov12 */
...
/* read the dimension of the aerosol climatology */
  ...
  if (cloudcalc != 0){ 
    printf("dedicated to work with the n file, xndim= %d yndim= %d \n", xndim,yndim);
	...
	if(aperture=99)  
	  {
	  //read_grid_header(HDL,nfile,&cloud.header[0]);
	  fread(&headline[0],HDL,1,cloud.f);
	  //printf("cloud header %s \n \n", headline[0]);
	  rheadinfo(&headline[0],&hinfo);
	  doy=hinfo.date;
	  hour=hinfo.aq_time/100;
	  minute=fmod(hinfo.aq_time,100);
	  // bildfolge[anzahl].doy=160;
	  //hour=1;
	  // min=25;
	  yndim=hinfo.nrlines;
	  xndim=hinfo.nrcolumns;
	  //hinfo.latbegin=-62.0;
	  headleng=HDL;
	  printf("Warning: some information of the config file is overwritten by header information, this is ok when standard binary files with regular lat lon grids are used \n ");
	  printf("the doy, hour, minute, ydim, xdim from header %d %d %d %d %d \n", doy,hour,minute, yndim, xndim);
	  printf("latb lonb %lf %lf %lf %lf %lf %lf  \n ", hinfo.latbegin, hinfo.latend,  hinfo.lonbegin, hinfo.lonend, hinfo.dlat, hinfo.dlon);
	  // exit(0);
	  }       
	...
	switch (bsize)
	  {
	  case 1: {
          cloud.n=bmatrix(0,yndim-1,0,xndim-1);
	  ...
	  for (lin=0;lin<yndim;lin++){
		...
		cloud.in[lin][col]=4*(short int)cloud.n[lin][col]; /* RM 05.11.2010 & before cloud.in is removed, 
                as it made no sense !! */
	    }
	  }
  ...
  for (lin=0;lin<ydim;lin++){  /* lin=line (lat,yvalues)*/
      ...
      if (cloudcalc != 0)
	{
	  ...
	  if (aperture == 99)
	    { // decide=0;
	      xlon=(hinfo.lonbegin+col*hinfo.dlon);  // needed as dlon is given as int in header        
	      ylat=(hinfo.latend-lin*hinfo.dlat);
	      //printf("xdim, xlon, ydim, ylat LIN,COL CAL = %d %f %d %f %d %d %f \n", xdim, xlon, ydim, ylat, lin, col, cloud.in[lin][col]);
	    }
	  else	
	    {
	      geoloc(rlin,rcol,yndim,&ylat,&xlon);
	    } /* geolocation for Meteosat images */
      ...
      else
	{
	 ...
	 clear.g[lin][col]=-1;
         printf("setting gclear to -1, ylat= %d xlon = %d", ylat, xlon);
       }
       else{ 
   ...
   /* printf("1st terms %f \n",alut.ag[1][kls][i]);
	...
	hence no correction for Bmlb !! */
	 ...
	 /* printf("hello here comes the ne stuff"); */
         switch (bsize){
         case 1: drange=1024;  
	     /* RM 05.11-2010, the value of 256 was wrong and has been replaced
                A Value of 256 leads to dramatically lower solar irradiance !! */ 
	   break;
	 case 2: drange=1024;
           break;
...
/* free the fields that are no longer needed */
   ...
   */
    ...
    if (regrid != 0 && cloudcalc != 0 && aperture != 99)
     {
    ...
    if (aperture==99)
      {
	printf("write output for regular grid, all sky xdim= %d, ydim=%d \n", xdim, ydim);
	lonreg=vector(0,xdim-1);
	latreg=vector(0,ydim-1);
	for (i=0;i<xdim;i++){
	  lonreg[i]=hinfo.lonbegin+i*hinfo.dlon;
	}
	for (j=0;j<ydim;j++){ 
	  latreg[j]=hinfo.latend-j*hinfo.dlat;
	  /* printf("latreg %f \n", latreg[j]); */
	}
	printf("before cdlout \n");
	fcdloutCS(lonreg,latreg,xdim,ydim,cloud.g,cloud.b,cdlfile);
	printf("after ...\n");
...
if (!(in_image=fopen(image_name,"rb")))   /* das Bild muss zum Lesen zu oeffnen sein            */
  {printf("ERROR: image %s not available for reading",image_name );
    exit(0);
  } 
 /*

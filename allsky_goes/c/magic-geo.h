...
/* added struct headinfo, defines variables of the standard MAGICSOL header, regular grid data */
struct headinfo{
    unsigned long nrlines ; // number of lines
    unsigned long nrcolumns ; // number of columns
    float latbegin ; // latitude of the region, defined N->S (90->-90), starting North
    float lonbegin ; // longitude of the region, defined W->E (-180->180), starting West
    float dlat ; // grid size in degree latitude of the regular lat/lon grid
    float dlon ; // grid size in degree longitude of the regular lat/lon grid
    float latend ; // 
    float lonend ; //
    float linoff ; // offset in lines  induced by latbegin
    float coloff ; // offset in columns induced by lonbegin
    unsigned long aq_time ; // time in ...
    unsigned long date ; // date YYYYDOY
    unsigned long scandur ; // duration of scna in minutess MM
    unsigned long dummy1;
    unsigned long dummy2;
    char blub[196] ; // not needed the rest of the 256 byte header
};
...
/* Nov 12, added in order to read the "standard" MAGICSOL header */
void read_grid_header(short int head_length, char *image_name, unsigned char *headline);

void rheadinfo(unsigned char *headline,struct headinfo *temp);

/* read the standart header file defined by rich from other satellites whereby the
   data has been transformed to a regular lat,lon grid beforehand */

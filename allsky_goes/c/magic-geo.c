...
/* added function to read MAGICSOL standard header, 4 regular grid GOES data */
...
{
    (*temp).nrlines=*((unsigned long *)(&headline[0]));
    (*temp).nrcolumns=*((unsigned long *)(&headline[4]));
    (*temp).latbegin=*((float *)(&headline[8])); 
    (*temp).lonbegin=*((float *)(&headline[12]));
    (*temp).dlat=*((float *)(&headline[16])); 
    (*temp).dlon=*((float *)(&headline[20]));
    (*temp).latend=*((float *)(&headline[24]));
    (*temp).lonend=*((float *)(&headline[28]));
    (*temp).linoff=*((float *)(&headline[32]));
    (*temp).coloff=*((float *)(&headline[36]));
    (*temp).aq_time=*((unsigned long *)(&headline[40]));  
    (*temp).date=*((unsigned long *)(&headline[44]));
    (*temp).scandur=*((unsigned long *)(&headline[48]));
}

...
/* 12.10.09 included new lines for the calculation of GMT. Now MFG and MSg can be processed */
 ...
 if (scata*r2d<15.0)
 {
     esd=1.0+0.035*(scata*r2d-15.0)/15.0;
 }
 else if (scata*r2d >= 15.0 && scata*r2d <= 33.0 && msza*r2d <40.0)
 {  
     esd=1.02;
 }
 else if (scata*r2d >34.0 && scata*r2d < 50.0)
 {
     esd=0.985+0.03*fabs(37.0-scata*r2d)/9;
 }
 else if (scata*r2d >= 50.0 && scata*r2d <= 70.0) 
 {
     esd=1.0+0.03*(63.0-scata*r2d)/15.0  ;
 }
 else 
 {
     esd=1.0;
 }
 if (msza*r2d > 80) esd=1.0;  /* esd =1 in all cases not covered by previous cases and for SZA>80 */
 ...
 fprintf(fstatrho, "# msza= %f mazi= %f satazi= %f satzen= %f scattangle= %f rhomax %f \n",  msza*r2d, (mazi)*r2d, msatazi*r2d, msatzen*r2d, scata*r2d, fper[pm]*esd*1.1);
